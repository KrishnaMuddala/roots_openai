...
class Openai:
    ...
    async def chat_stream(
        ...
        # Collect streamed chunks into a final _FakeMessage.
        # self.client is synchronous OpenAI(), so we use a regular for loop.
        # Being inside an async method still allows await on_event(chunk).
        collected_content = ""
        ...
        # Build a minimal fake OpenAI-style response so _FakeMessage can parse it.
        tool_calls_list = [
            _FakeToolCallResult(
                tc["id"],
                tc["function"]["name"],
...
# ---------------------------------------------------------------------------
# Helpers used by chat_stream to reconstruct a fake OpenAI response object
# ---------------------------------------------------------------------------
